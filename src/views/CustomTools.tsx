  ...
  const startScan = async () => {
    ...
    try {
      if (!('bluetooth' in navigator)) {
        setMessage('Cannot connect. Make sure Bluetooth is turned on and supported by your browser.');
        setScanning(false);
        return;
      }
      setScanning(true);
      ...
      setMessage('Connected successfully to ' + (device.name || 'Unknown Device'));
      setScanning(false);
    } catch (error: any) {
  ...
  return (
    <CustomToolLayout tool={tool} onClose={onClose}>
      <div className="flex flex-col h-full space-y-4">
        <label className="text-gray-500 text-[10px] font-bold uppercase tracking-widest block">BLUETOOTH SCANNER</label>
        
        <div className="flex flex-col items-center justify-center border border-neon-green/20 bg-neon-green/5 rounded-2xl p-8 flex-1 min-h-[300px] text-center">
          <div className={`w-24 h-24 rounded-full border-2 ${scanning ? 'border-neon-green animate-pulse' : 'border-neon-green/30'} flex items-center justify-center mb-6`}>
             <span className="text-4xl text-neon-green font-bold">BT</span>
          </div>
          
          <p className="text-xs text-gray-400 font-mono mb-8 max-w-xs leading-relaxed">
            {message || 'Click the button below to pair a nearby Bluetooth Low Energy device with your device.'}
          </p>

          <button onClick={startScan} disabled={scanning} className="bg-neon-green/10 text-neon-green border border-neon-green/30 hover:bg-neon-green hover:text-black transition-all px-8 py-4 w-full sm:w-auto rounded-xl font-bold uppercase tracking-widest text-xs disabled:opacity-50">
             {scanning ? 'SCANNING...' : 'PAIR DEVICE'}
          </button>
        </div>

        {devices.length > 0 && (
           <div className="bg-[#050505] rounded-xl border border-neon-green/20 p-4 space-y-4 overflow-auto min-h-[150px]">
             <h3 className="text-xs font-bold text-gray-500 tracking-widest uppercase">Saved Devices</h3>
             {devices.map((d, i) => (
                <div key={i} className="flex justify-between items-center border-b border-neon-green/10 pb-2 last:border-0">
                  <div className="flex flex-col">
                    <span className="text-neon-green font-bold text-sm">{d.name || 'Unknown Name'}</span>
                    <span className="text-gray-400 text-[10px] font-mono">{d.id}</span>
                  </div>
                </div>
             ))}
           </div>
...
export function CveTool({ tool, onClose }: { tool: ToolDef, onClose: () => void }) {
  ...
  useEffect(() => {
    if (cachedRecentCves && cachedRecentCves.length > 0) {
      setRecentCves(cachedRecentCves);
      return;
    }
...
export function ExploitdbTool({ tool, onClose }: { tool: ToolDef, onClose: () => void }) {
  ...
  const searchExploits = async () => {
    ...
    try {
      const backendUrl = getBackendUrl();
      const res = await fetch(`${backendUrl}/api/net/exploitdb/search?q=${encodeURIComponent(keyword)}`);
